...
def cost_func(nn_param,
  ...
  a1 = np.c_[np.ones(num_samples), X].transpose()
  ...
  if b_y is None or size_y != y.shape:
    size_y = y.shape
  ...
  J = -(v_b_y.dot(v_hx) + (1 - v_b_y).dot(v_minus_hx))/ num_samples
  ...
  D3 = a3 - b_y.transpose()
  D2 = theta_2.transpose().dot(D3) * np.r_[np.zeros((1, z2.shape[1])), sigmoid_gradient(z2)]
  D2 = D2[1:,:]

  theta_1_grad = D2.dot(a1.transpose())
  theta_2_grad = D3.dot(a2.transpose())
